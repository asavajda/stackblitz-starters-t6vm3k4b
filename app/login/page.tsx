...
export default function LoginPage() {
  ...
  const [email, setPassword_email] = useState('')
  ...
  async function handleResetPassword() {
    if (!resetEmail) { setErrore('Inserisci la tua email'); return }
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="email" value={email}
              onChange={e => setPassword_email(e.target.value)}
          ...
          <div className="border-t border-gray-100 pt-4">
            {!mostraReset ? (
              <button onClick={() => setMostraReset(true)}
                className="w-full text-sm text-gray-400 hover:text-gray-600 text-center">
                Password dimenticata?
              </button>
            ) : !resetInviato ? (
              <div className="space-y-3">
                <p className="text-sm text-gray-500 text-center">Inserisci la tua email per ricevere il link di reset</p>
                <input type="email" value={resetEmail} onChange={e => setResetEmail(e.target.value)}
                  placeholder="La tua email"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-300" />
                <div className="flex gap-2">
                  <button onClick={() => setMostraReset(false)}
              ...
              <p className="text-sm text-green-600 text-center">✓ Email inviata — controlla la tua casella di posta</p>
            )}
          </div>
        </div>
      </div>
